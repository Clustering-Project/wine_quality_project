...
from explore import cluster_alc_dens, new_feats, cluster_two
...
def cluster_two_plt(df):
    df = new_feats(df)
    df = cluster_two(df)
    plt.figure(figsize=(8, 6))
    sns.scatterplot(data=df, x='total_acidity', y='alcohol_by_density', hue='cluster_labels', palette='viridis', alpha=0.6)
    plt.title('K-Means Clustering of Total Acidity vs. Alcohol by Density')
    plt.xlabel('Total Acidity')
    plt.ylabel('Alcohol by Density')
